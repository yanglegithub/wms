...
//						String date[]=wmomNoticeIEntity.getGoodsId().split("-");
						...
						if(wmOmNoticeH.getCusCode()==null){
							if(StringUtil.isNotEmpty(wmUtil.getCusCode())){
								wmomNoticeIEntity.setCusCode(wmUtil.getCusCode());
							}
						}else{
							wmomNoticeIEntity.setCusCode(wmOmNoticeH.getCusCode());
						}
			...
			boolean ok = wmOmNoticeHService.wmsAddMain(wmOmNoticeH, wmomNoticeIListnew);
			if(!ok) {
				j.setSuccess(false);
				j.setMsg("库存不足");
				return j;
			}
			/*for(WmOmNoticeIEntity wmOmNoticeI:wmomNoticeIListnew) {
				String hql = "from MdPalletEntity mp where mp.tuoPanZhuangTai = ? AND mp.tuoPanBianMa IN ('"+wmOmNoticeI.getBinId().replaceAll(",","','")+"') ORDER BY mp.binBianMa,mp.binDepth desc DESC;";
                ...
                for (MdPalletEntity pallet : pallets){
                    ...
                    movePallet.setRecord(pallet.getBinBianMa() + "->" + wmOmNoticeH.getOmPlatNo());
