...
//							wmImNoticeI.setGoodsUnit(mdgoods.getShlDanWei());
	...
	public List<WmImNoticeIEntity> findByNoticeIdAndStaus(String noticeId, String status) {
 		String hql = "from WmImNoticeIEntity i where i.imNoticeId=? and i.noticeiSta=?";
		List<WmImNoticeIEntity> result = findHql(hql,noticeId,status);
		return result;
	}

	@Override
	public List<MdPalletEntity> findPalletsById(String id) {
 		String hql = " from MdPalletEntity mp where mp.entryKey=?";
		List<MdPalletEntity> pallets = this.findHql(hql, id);
		return pallets;
	}

	@Override
	public List<WmImNoticeHPage> findConformedImHList(String imKey) {
 		String hql = "from WmImNoticeHEntity wmh where wmh.imSta=? and wmh.orderTypeCode=? and wmh.noticeId like ?";
		List<WmImNoticeHEntity> hlist = this.findHql(hql, WmsContants.CONFIRMED, "01", "%"+imKey+"%");
		List<WmImNoticeHPage> pages = new ArrayList<>();
		for (WmImNoticeHEntity h : hlist){
			WmImNoticeHPage page = new WmImNoticeHPage();
			List<WmImNoticeIEntity> is = this.findByProperty(WmImNoticeIEntity.class, "imNoticeId", h.getNoticeId());
			try {
				MyBeanUtils.copyBeanNotNull2Bean(h, page);
				if(page != null)
					page.setWmImNoticeIList(is);
				pages.add(page);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	@Override
	public void wmsAddMain(WmImNoticeHEntity wmImNoticeH, List<WmImNoticeIEntity> wmImNoticeIList) {
		//保存主信息
		this.save(wmImNoticeH);
		...
		for(WmImNoticeIEntity wmImNoticeI:wmImNoticeIList){
			...
			if(mvgoods!=null){
				...
				try {
					...
					wmImNoticeI.setGoodsUnit(mvgoods.getShlDanWei());
					wmImNoticeI.setBaseUnit(mvgoods.getBaseunit());
					wmImNoticeI.setGoodsBatch(getBatchNo(wmImNoticeI.getGoodsCode()));
//						wmImNoticeI.setGoodsWeight(String.valueOf(Long.parseLong(mdgoods.getZhlKg())*Long.parseLong(wmImNoticeI.getGoodsCount())));
//						wmImNoticeI.setGoodsUnit(mdgoods.getShlDanWei());
				...
				try {
					//wmImNoticeI.setGoodsWeight(String.valueOf(Double.parseDouble(mvgoods.getZhlKg())*Double.parseDouble(wmImNoticeI.getGoodsCount())));
					Double guige = Double.parseDouble(wmImNoticeI.getGoodsWeight())/Long.parseLong(wmImNoticeI.getGoodsCount());
					wmImNoticeI.setShpGuiGe(guige.toString());
				} catch (Exception e) {
					// TODO: handle exception
				}
			...
			if("04".equals(wmImNoticeH.getOrderTypeCode())){//越库任务
				...
				wmImNoticeI.setImBeizhu(wmImNoticeH.getImBeizhu());
				wmImNoticeI.setImCusCode(wmImNoticeH.getImCusCode());
				wmImNoticeI.setNoticeiSta(wmImNoticeH.getImSta());
				wmImNoticeI.setBpmSta(BpmStatus.CONFIRMING);
				wmImNoticeI.setGoodsQmCount("0");
				wmImNoticeI.setGoodsWqmCount(wmImNoticeI.getGoodsCount());
				this.save(wmImNoticeI);
			}
	 ...
	 */
 	...
 	public String getBatchNo(String goodsCode){
 		SimpleDateFormat format = new SimpleDateFormat("yyyyMMdd");
		Date today = new Date();
		String prefix = "GB"+format.format(today);
		String sql = "select count(*) from WmImNoticeIEntity imi where imi.createDate >= ? and imi.goodsCode=?";
		Long count = this.getCountForJdbcParam(sql, new Object[]{today,goodsCode});
